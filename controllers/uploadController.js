...
exports.create = async (req, res) => {
  const files = [];
  try {
    if (Array.isArray(req.files.uploads) && req.files.uploads.length > 0) {  //checking if req.files.uploads is array and it exist or not
  ...
  for (var k = 0; k < files.length; k++) {  //loop to iterate through all files in files array
    ...
    for (var i = 0; i < obj.length; i++) {
      var sheet = obj[i];
      for (var j = 0; j < sheet["data"].length; j++) {  //pushing each sheet's data to rows
      ...
      fs.createReadStream(path1)          //reading the file on path - path1
        ...
        .on("end", async (rowCount) => {
          ...
          try {
            ...
            chmodr("./", 0o777, (err) => {    //giving permission to read,write and execute to current folder
              if (err) {
